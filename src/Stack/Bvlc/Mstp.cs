...
internal class Mstp
{
    ...
    public const ApduMax MSTP_MAX_APDU = ApduMax.MAX_APDU480;
    ...
    public static int Encode(byte[] buffer, int offset, MstpFrameTypes frameType, byte destinationAddress, byte sourceAddress, int msgLength)
    {
    ...
    public static int Decode(byte[] buffer, int offset, int maxLength, out MstpFrameTypes frameType, out byte destinationAddress, out byte sourceAddress, out int msgLength)
    {
        if (maxLength < MSTP_HEADER_LENGTH)  //not enough data
        {
            frameType = MstpFrameTypes.FRAME_TYPE_BACNET_DATA_EXPECTING_REPLY; // don't care
            destinationAddress = sourceAddress = 0;   // don't care
            msgLength = 0;
            return -1;
        }

        frameType = (MstpFrameTypes)buffer[offset + 2];
